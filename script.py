...
print("Welcome to the file converter! Here, you can convert YouTube videos to "
      "wav files for audio along with m4a, and include timestamps as well. "
      "Please, type 'quit' at anytime if you wish to exit.")
...
if if_playlist:
    all_vids = True
    range_exists = input("Would you like to download only specific videos of "
                         "the playlist? Enter 'yes' or 'no'. ")

    range_input = functions.check_input(range_exists)
    while not range_input:
        range_exists = input("Invalid answer. Would you like to download only "
                             "specific videos of the playlist? Enter 'yes' or"
                             " 'no'. ")
        range_input = functions.check_input(range_exists)
    ...
    if range_exists == 'yes':
        ...
        vid_range = input("Enter the videos you would like to download, "
                          "or enter '0' if you want all: Ex. '1 3 7-9'. ")
    ...
    ts = input("Are there any timestamps in which you would like to separate "
               "the video? Enter 'yes' or 'no'. ")
    ...
    if if_timestamps:
        ...
        input("Thank you for using the converter! Just use 'python3 "
              "script.py' if you ever wish to use this again. Click 'Enter' to exit.")
        sys.exit()
    else:
